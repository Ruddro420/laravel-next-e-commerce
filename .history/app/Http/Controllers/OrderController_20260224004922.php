   ...
   private function buildItemsSafe(array $items): array
{
    ...
    \Log::info('buildItemsSafe - INCOMING ITEMS:', $items);
    ...
    // 🚨 DEBUG: Log variant IDs found
    \Log::info('buildItemsSafe - Variant IDs found:', $variantIds->toArray());
    ...
    foreach ($items as $it) {
        ...
        \Log::info('buildItemsSafe - Processing item:', [
            'product_id' => $pid,
            'variant_id' => $vid,
            'variant_id_exists' => isset($it['variant_id']),
            'variant_id_value' => $it['variant_id'] ?? null,
            'raw_item' => $it
        ]);
        ...
        if ($vid && (!$v || ($pid && (int)$v->product_id !== (int)$pid)) {
            // 🚨 DEBUG: Log variant validation failure
            \Log::warning('buildItemsSafe - Variant validation failed:', [
                'variant_id' => $vid,
                'variant_exists' => !!$v,
                'variant_product_id' => $v ? $v->product_id : null,
                'product_id' => $pid
            ]);
        ...
        if (Schema::hasColumn('order_items', 'variant_id')) {
            $row['variant_id'] = $vid;
            // 🚨 DEBUG: Log when adding variant_id to row
            \Log::info('buildItemsSafe - Adding variant_id to row:', [
                'variant_id' => $vid,
                'row' => $row
            ]);
        } else {
            // 🚨 DEBUG: Log if column doesn't exist
            \Log::warning('buildItemsSafe - variant_id column does not exist in order_items table');
        }
    ...
    \Log::info('buildItemsSafe - FINAL PAYLOAD:', $payload);
