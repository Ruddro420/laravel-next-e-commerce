...
<script>
  // helper (you already call escapeHtml but didn't define it)
  ...
  function toggleProductTypeUI() {
    ...
    // default: hide all special blocks
    ...
    if (type === 'simple') {
      ...
      // pricing is per-variant in variable products, so keep simpleBlock hidden
    }
  }

  function makeAttrRow(index) {
    const row = document.createElement('div');
    row.className = "grid grid-cols-1 md:grid-cols-12 gap-3 items-end rounded-2xl border border-slate-200 p-3 dark:border-slate-800";
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    toggleProductTypeUI();
    productType?.addEventListener('change', toggleProductTypeUI);

    // add first row automatically when switching to variable (optional)
