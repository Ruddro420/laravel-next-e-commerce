...
(function(){
  ...
  // for edit page load saved json
  const EXISTING = @json(old('var_attributes', $product->variable_attributes_json ?? []));
  ...
  async function loadValues(attributeId, selectEl, selectedIds = []){
    ...
    const url = `{{ route('products.attributeValues') }}?attribute_id=${attributeId}`;
