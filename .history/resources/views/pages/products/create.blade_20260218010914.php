            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="text-xs text-slate-500 dark:text-slate-400">
                        Select attributes and choose options (example: Color → Red, Blue).
                    </div>
                </div>
...
(function(){
  ...
  const ATTRS = @json(($attributes ?? collect())->map(fn($a)=>['id'=>$a->id,'name'=>$a->name])->values());
      ...
      <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-800" data-attr-row>
        ...
        <div class="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <select name="var_attributes[${i}][attribute_id]" class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm dark:bg-slate-900 dark:border-slate-800" data-attr-select required>
              <option value="">Select attribute</option>
              ${ATTRS.map(a => `<option value="${a.id}">${a.name}</option>`).join('')}
            </select>
          </div>

          <div>
            <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">Options</label>
            <select name="var_attributes[${i}][option_ids][]" multiple
              class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm dark:bg-slate-900 dark:border-slate-800"
              data-opt-select required>
  ...
  async function loadOptions(attributeId, optSelect){
    optSelect.innerHTML = `<option value="">Loading...</option>`;
    const url = `{{ route('products.attributeOptions') }}?attribute_id=${attributeId}`;
    const res = await fetch(url, { headers: { 'X-Requested-With':'XMLHttpRequest' }});
    const data = await res.json();

    optSelect.innerHTML = data.length
      ? data.map(o => `<option value="${o.id}">${o.value}</option>`).join('')
  ...
  attrWrap?.addEventListener('click', (e)=>{
    ...
    if(!rm) return;
    rm.closest('[data-attr-row]').remove();
  });
  ...
  attrWrap?.addEventListener('change', (e)=>{
    ...
    const optSelect = row.querySelector('[data-opt-select]');
    if(!sel.value){ optSelect.innerHTML = `<option value="">Select attribute first</option>`; return; }
    loadOptions(sel.value, optSelect);
  });
