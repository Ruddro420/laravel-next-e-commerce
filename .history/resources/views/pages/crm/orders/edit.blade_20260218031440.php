{{-- resources/views/pages/crm/orders/edit.blade.php --}}
@extends('layouts.app')
...
<div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft dark:bg-slate-900 dark:border-slate-800">
  ...
  @php
    // Build JS-friendly product list (fixes $productsJs undefined)
  ...
  <form method="POST" action="{{ route('crm.orders.update',$order) }}" class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        ...
        <div class="mt-1 text-xs text-slate-500 dark:text-slate-400">Changing customer will auto-fill addresses below.</div>
      </div>
    ...
    <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
      <div class="flex items-center justify-between">
        <div>
          ...
          <div class="text-xs text-slate-500 dark:text-slate-400">Product dropdown will auto-fill name, sku and price.</div>
        </div>
      ...
      <div id="stockError"
        class="hidden mt-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-200"></div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div>
        ...
        <input id="shipping" name="shipping" type="number" step="0.01"
    ...
    <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
      ...
      <div class="mt-3 grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <input id="amountPaid" name="payment[amount_paid]" type="number" step="0.01"
            value="{{ $paid }}"
            class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm dark:bg-slate-900 dark:border-slate-800" />
        </div>
      </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // IMPORTANT: make sure items are plain array
  const existingItems = @json($order->items->values());
  ...
  // data may have product_id null; still show row with stored product_name/sku/price
      ...
      <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-800" data-item>
        ...
        <div class="mt-3 grid grid-cols-1 md:grid-cols-6 gap-3">
          ...
          <div class="md:col-span-2">
            ...
            <input name="items[${i}][product_name]" data-name required value="${escapeHtml(pname)}"
              class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm dark:bg-slate-900 dark:border-slate-800" />
            <div class="mt-1 text-[11px] text-slate-500 dark:text-slate-400">If product_id missing, select product to sync.</div>
          </div>
          ...
          <div>
            ...
            <input name="items[${i}][price]" type="number" step="0.01" min="0" value="${price.toFixed(2)}" required data-price
              class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm dark:bg-slate-900 dark:border-slate-800" />
          </div>
  ...
  function syncRowFromProduct(itemEl){
    ...
    // overwrite name/sku/price based on selected product
  ...
  // when selecting product -> sync name/sku/price
  wrap.addEventListener('change', (e)=>{
    if(e.target.matches('[data-product]')){
      const item = e.target.closest('[data-item]');
      syncRowFromProduct(item);
      calc();
    }
  });
  ...
  // render existing items (THIS IS THE MAIN FIX)
  if(Array.isArray(existingItems) && existingItems.length){
    ...
    // after rendering, if any row has product_id -> sync displayed stock label + maybe override fields
    wrap.querySelectorAll('[data-item]').forEach(item=>{
      const pid = item.querySelector('[data-product]')?.value || '';
      if(pid) syncRowFromProduct(item);
    });
