  ...
  <form method="POST" action="{{ route('crm.orders.store') }}" class="space-y-6">
    ...
    <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
      <div class="flex items-center justify-between">
        <div>
          ...
          <div class="text-xs text-slate-500 dark:text-slate-400">Pick product, qty and price. Stock will update when you save.</div>
        </div>
      ...
      <div id="stockError"
        class="hidden mt-3 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800 dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-200">
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        ...
        <input id="couponCode" name="coupon_code" value="{{ old('coupon_code') }}"
          class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm dark:bg-slate-900 dark:border-slate-800"
    ...
    @php
      ...
      $paid = old('payment.amount_paid',0);
    @endphp
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!wrap || !btnAdd) {
    console.error('Order Items DOM not found:', { wrap, btnAdd });
    return;
  }
  ...
  function showStockError(msg){
    if(!stockError || !btnSubmit) return;
  ...
  function calc(){
    ...
    if(sumDue) sumDue.textContent = due.toFixed(2);
  ...
  // On product select -> auto-fill fields
  ...
  function syncPay(){
    ...
    if(trxWrap) trxWrap.classList.toggle('hidden', !needTrx);
  }
  if(method) method.addEventListener('change', syncPay);
  syncPay();
