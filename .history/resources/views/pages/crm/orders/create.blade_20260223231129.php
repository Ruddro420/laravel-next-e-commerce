    ...
    <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
      <div class="flex items-center justify-between">
        <div>
          ...
          <div class="text-xs text-slate-500 dark:text-slate-400">Select product → variant → attributes.</div>
        </div>
...
@php
  // Build products data (with variants + attributes) for JS
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function variantOptionsHtml(product, selectedVariantId = ''){
    if(!product) return `<option value="">Select variant</option>`;
    const vars = Array.isArray(product.variants) ? product.variants : [];
    if(vars.length === 0) return `<option value="">No variants</option>`;

    let html = `<option value="">Select variant</option>`;
    vars.forEach(v => {
      const sel = String(selectedVariantId) === String(v.id) ? 'selected' : '';
      html += `<option value="${v.id}" data-price="${v.price ?? 0}" data-sku="${escapeHtml(v.sku||'')}" ${sel}>
        ${escapeHtml(v.name || ('Variant #'+v.id))}${v.sku ? ' — '+escapeHtml(v.sku) : ''}${(v.price && Number(v.price)>0) ? ' (৳'+Number(v.price).toFixed(2)+')' : ''}
      </option>`;
    });
    return html;
  }
        ...
        ${attrs.map((a, idx) => {
          ...
          return `
            <div>
              ...
              <select name="items[${i}][attributes][${escapeHtml(key)}]" data-attr-select
                class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm dark:bg-slate-900 dark:border-slate-800">
  ...
  function row(i, data = null){
    ...
    return `
      <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-800" data-item data-index="${i}">
        <div class="flex items-center justify-between">
        ...
        <div class="mt-3 grid grid-cols-1 md:grid-cols-8 gap-3">
          <div class="md:col-span-2">
            <label class="text-xs font-semibold text-slate-600 dark:text-slate-300">Product</label>
            <select name="items[${i}][product_id]" data-product required
              class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm dark:bg-slate-900 dark:border-slate-800">
          ...
          <div class="md:col-span-2">
            ...
            <div class="mt-1 text-xs text-slate-500 dark:text-slate-400">If product has variants, select one.</div>
          </div>
        ...
        <div class="mt-2" data-attrs-wrap>
          ${renderAttributes(i, (productMap.get(String(pid))?.attributes || []), selectedAttrs)}
        </div>

        <div class="mt-2 text-xs text-slate-500 dark:text-slate-400">
          POS Tip: Variant price (if selected) becomes base price, attributes add extra price.
        </div>
      </div>
    `;
  }
  ...
  function applyAttributePrice(itemEl){
    ...
    // base price = variant price (if chosen) else product price
    ...
    if(variantSel && variantSel.value){
      ...
      const vPrice = Number(opt?.dataset.price || 0);
      if(vPrice > 0) base = vPrice;
    }

    let add = 0;
    itemEl.querySelectorAll('[data-attr-select]').forEach(sel => {
      const opt = sel.options[sel.selectedIndex];
      add += Number(opt?.dataset.price || 0);
    });
  ...
  function fillFromProduct(itemEl){
    ...
    if(!p){
      ...
      attrsWrap.innerHTML = '';
      if(variantSel) variantSel.innerHTML = `<option value="">Select variant</option>`;
      return;
    }

    // Fill base from product
    nameEl.value = p.name || '';
    skuEl.value = p.sku || '';
    ...
    // Variants list
    if(variantSel){
      variantSel.innerHTML = variantOptionsHtml(p, '');
    }

    // Attributes UI
    const i = itemEl.dataset.index;
    attrsWrap.innerHTML = renderAttributes(i, p.attributes || [], {});
    applyAttributePrice(itemEl);
  }

  function applyVariant(itemEl){
    const variantSel = itemEl.querySelector('[data-variant]');
    if(!variantSel) return;

    const opt = variantSel.options[variantSel.selectedIndex];
    const vSku = opt?.dataset.sku || '';
    const vPrice = Number(opt?.dataset.price || 0);

    // If variant has SKU/Price override, apply them
    if(vSku) itemEl.querySelector('[data-sku]').value = vSku;
    if(vPrice > 0) itemEl.querySelector('[data-price]').value = vPrice.toFixed(2);
  ...
  function addRow(data=null){
    ...
    // If old data contains product_id, populate dependent UI
    const pid = itemEl.querySelector('[data-product]')?.value;
    if(pid){
      const p = productMap.get(String(pid));
      if(p){
        // Set variants list and select old variant if present
        const variantSel = itemEl.querySelector('[data-variant]');
        if(variantSel){
          const oldVid = data?.variant_id ?? '';
          variantSel.innerHTML = variantOptionsHtml(p, oldVid);
        }
        // Fill from product (name/sku/price) then apply old variant selection
        fillFromProduct(itemEl);
        if(data?.variant_id){
          itemEl.querySelector('[data-variant]').value = String(data.variant_id);
          applyVariant(itemEl);
        }
      }
    }
  ...
  // product/variant/attribute changed
