...
<div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-soft dark:bg-slate-900 dark:border-slate-800">
  ...
  <form method="POST" action="{{ route('crm.orders.update',$order) }}" class="space-y-6">
    ...
    @php
      ...
      // ✅ coupon + discount from DB
      $couponCode = old('coupon_code', $order->coupon_code ?? '');
      $discountFromOrder = (float)(old('discount', $order->coupon_discount ?? $order->discount ?? 0));
    @endphp
  ...
  <option value="cash_received" {{ $pm==='cash_received'?'selected':'' }}>
    Cash Received
  </option>
  ...
  <option value="bkash" {{ $pm==='bkash'?'selected':'' }}>
    bKash
  </option>

  <option value="nagad" {{ $pm==='nagad'?'selected':'' }}>
    Nagad
  </option>

  <option value="rocket" {{ $pm==='rocket'?'selected':'' }}>
    Rocket
  </option>
</select>
        </div>
    ...
    <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-800">
      ...
      <div class="flex flex-wrap md:flex-nowrap items-center justify-between gap-6 text-sm">
        ...
        <div class="flex flex-col">
          <span id="sumDiscountLabel" class="text-xs text-emerald-600 dark:text-emerald-400">Discount</span>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const discountFixed = Number(@json($discountFromOrder)) || 0;
  ...
  const sumDiscountLabel = document.getElementById('sumDiscountLabel');
  ...
  itemsWrap.querySelectorAll('[data-item]').forEach(item => {
    ...
    const lineInput = item.querySelector('[data-line]');
  ...
  if (rate > 0) {
    if (mode === 'exclusive') {
      ...
      tax = base - (base / divisor);
    }
  }

  // ===============================
  // 4️⃣ Total BEFORE Discount
  // ===============================
  const totalBeforeDiscount =
    (mode === 'inclusive') ? base : (base + tax);
  ...
  // Auto full paid if Cash Received
  ...
  if (due <= 0) {
    ...
    const discount = Math.max(0, Math.min(discountFixed, totalBeforeDiscount));
    ...
    sumDiscountLabel.textContent = couponCode ? `Discount (${couponCode})` : 'Discount';
