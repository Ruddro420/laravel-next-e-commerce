...
<div id="modalCreate" class="hidden fixed inset-0 z-50">
    {{-- overlay that scrolls --}}
    <div class="absolute inset-0 bg-slate-900/40"></div>

    {{-- scroll container --}}
    <div class="relative h-full w-full overflow-y-auto p-4 md:p-8">
        <div class="mx-auto w-full max-w-3xl rounded-2xl bg-white p-5 shadow-soft dark:bg-slate-900">
            <div class="flex items-center justify-between">
                <div class="font-semibold">Add Role</div>
                <button type="button"
                    class="h-9 w-9 rounded-xl border border-slate-200 dark:border-slate-800"
                    data-close>✕</button>
            </div>
            ...
            <form method="POST" action="{{ route('settings.roles.store') }}" class="mt-4 space-y-4">
                ...
                <div>
                    ...
                    {{-- inner scroll area (optional but nice) --}}
                    <div class="mt-3 max-h-[60vh] overflow-y-auto pr-1 space-y-4">
        ...
        document.querySelectorAll('[data-edit]').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                document.querySelectorAll('.editPerm').forEach(ch => {
                    ch.checked = (data.permIds || []).includes(parseInt(ch.value));
                });
