...
(function(){
  ...
  // ---------- State ----------
  // key: "p:ID" for simple/downloadable, "v:VARIANT_ID" for variants
  const cart = new Map();
  ...
  document.getElementById('btnCloseVariant')?.addEventListener('click', closeVariantModal);
  document.getElementById('btnCancelVariant')?.addEventListener('click', closeVariantModal);
  ...
  function renderResults(items){
    results.innerHTML = '';
    if(!items.length){
      results.innerHTML = `<div class="col-span-full text-sm text-slate-500 dark:text-slate-400">No products found.</div>`;
      return;
    }
    ...
    items.forEach(p=>{
      ...
      results.insertAdjacentHTML('beforeend', `
        <button type="button" data-add="${p.id}"
          ...
          <div class="mt-2 text-xs font-semibold ${actionColor} group-hover:underline">${actionText}</div>
        </button>
      `);
    });
  ...
  function openVariantModal(product){
    vmTitle.textContent = product.name + ' — Variants';
    vmSub.textContent = 'Select a variant to add to cart.';
    variantList.innerHTML = '';

    const variants = Array.isArray(product.variants) ? product.variants : [];

    if(!variants.length){
      variantList.innerHTML = `<div class="col-span-full text-sm text-slate-500 dark:text-slate-400">No variants found.</div>`;
      variantModal.classList.remove('hidden');
      return;
    }

    variants.forEach(v=>{
      const stockTxt = (v.stock === null || v.stock === undefined)
        ? 'Stock: —'
        : (Number(v.stock) <= 0 ? 'Out of stock' : 'Stock: ' + v.stock);

      const stockCls = (v.stock !== null && v.stock !== undefined && Number(v.stock) <= 0)
        ? 'text-rose-600'
        : 'text-slate-500 dark:text-slate-400';

      variantList.insertAdjacentHTML('beforeend', `
        <button type="button" data-choose-variant="${v.id}"
          class="rounded-2xl border border-slate-200 p-3 text-left hover:bg-slate-50 dark:border-slate-800 dark:hover:bg-slate-800">
          <div class="flex gap-3">
            <div class="h-12 w-12 rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-800 grid place-items-center">
              ${v.image ? `<img src="${v.image}" class="h-full w-full object-cover">` : `<span class="text-slate-400">🧩</span>`}
            </div>
            <div class="min-w-0 flex-1">
              <div class="font-semibold text-sm truncate">${escapeHtml(v.label || ('Variant #'+v.id))}</div>
              <div class="text-xs ${stockCls}">${stockTxt}</div>
              <div class="mt-1 text-sm font-bold">৳${money(v.price)}</div>
              <div class="text-[11px] text-slate-500 dark:text-slate-400 truncate">${escapeHtml(v.sku || '')}</div>
            </div>
          </div>
        </button>
      `);
    ...
    variantList.querySelectorAll('[data-choose-variant]').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        const vid = btn.getAttribute('data-choose-variant');
        const vv = variants.find(x => String(x.id) === String(vid));
        if(!vv) return;

        if(vv.stock !== null && vv.stock !== undefined && Number(vv.stock) <= 0){
          showMsg('error', 'Variant out of stock.');
          return;
        }

        addVariantToCart(product, vv, 1);
        closeVariantModal();
      });
    });
  ...
  // ---------- Cart add helpers ----------
  function addProductToCart(prod, qty){
    const key = `p:${prod.id}`;
  ...
  function addVariantToCart(product, variant, qty){
    ...
    if(variant.stock !== null && variant.stock !== undefined){
      const max = Number(variant.stock);
      const nextQty = (existing ? existing.qty : 0) + addQty;
      if(nextQty > max){
        showMsg('error', `Not enough stock for this variant. Available: ${max}`);
        return;
      }
    }
    ...
    if(existing){
      ...
      cart.set(key, {
        ...
        stock: (variant.stock === null || variant.stock === undefined) ? null : Number(variant.stock),
        qty: addQty,
      });
    }
  ...
  function renderCart(){
    ...
    Array.from(cart.values()).forEach(item=>{
      ...
      const label = item.variant_label ? ` <span class="text-xs text-slate-500 dark:text-slate-400">(${escapeHtml(item.variant_label)})</span>` : '';

      cartWrap.insertAdjacentHTML('beforeend', `
        <div class="rounded-2xl border border-slate-200 p-3 dark:border-slate-800" data-cart-item="${escapeHtml(item.key)}">
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
              <div class="font-semibold truncate">${escapeHtml(item.name)}${label}</div>
            ...
            <button type="button" data-remove="${escapeHtml(item.key)}" class="text-xs font-semibold text-rose-600">Remove</button>
          </div>
          ...
          <div class="mt-3 grid grid-cols-3 gap-2 items-end">
            <div>
              ...
              <input type="number" step="0.01" min="0" value="${money(item.price)}" data-price="${escapeHtml(item.key)}"
            ...
            <div>
              ...
              <input type="number" min="1" value="${item.qty}" data-qty="${escapeHtml(item.key)}"
                class="mt-1 w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm dark:bg-slate-900 dark:border-slate-800">
            </div>
    ...
    cartWrap.querySelectorAll('[data-price]').forEach(inp=>{
      inp.addEventListener('input', ()=>{
        const key = String(inp.getAttribute('data-price'));
        const item = cart.get(key);
        if(!item) return;
  ...
  function calcTotals(){
    ...
    const shipCost = Number(shipping.value || 0);
  ...
  document.querySelectorAll('[data-quickpay]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      amountPaid.value = Number(btn.getAttribute('data-quickpay') || 0);
      calcTotals();
    });
  });
  ...
  document.querySelectorAll('[data-loadhold]').forEach(btn=>{
    btn.addEventListener('click', async ()=>{
      ...
      const res = await fetch(`/pos/holds/${id}`, { headers:{ 'Accept':'application/json' }});
      const json = await res.json().catch(()=>null);
      if(!res.ok || !json?.ok){
      ...
      // restore cart: we fetch products list by searching product_id
      for(const it of (p.cart || [])){
        ...
        const prod = list.find(x => String(x.id) === String(pid));
        if(!prod) continue;

        if(vid){
          const vv = (prod.variants || []).find(v => String(v.id) === String(vid));
          if(vv) addVariantToCart(prod, vv, qty);
        }else{
  ...
  function startScan(){
    ...
    Quagga.onDetected(async function(data){
      ...
      if(exact.type === 'variable'){
        // If variable barcode is on parent product, open modal
        openVariantModal(exact);
        showMsg('info', 'Scanned variable product. Choose variant.');
      }else{
  ...
  document.addEventListener('keydown', (e)=>{
    if(e.key === 'F2'){ e.preventDefault(); btnScan.click(); }
    if(e.key === 'F4'){ e.preventDefault(); btnHoldSale.click(); }
