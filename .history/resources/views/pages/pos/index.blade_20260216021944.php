...
(function(){
  ...
  function renderResults(items){
    ...
    items.forEach(p=>{
      const stockTxt = (p.stock <= 0) ? 'Out of stock' : ('Stock: ' + p.stock);
      const stockCls = (p.stock <= 0) ? 'text-rose-600' : 'text-slate-500 dark:text-slate-400';
      ...
      results.insertAdjacentHTML('beforeend', `
        <button type="button" data-add="${p.id}"
          ...
          <div class="mt-2 text-xs font-semibold text-indigo-600 group-hover:underline">Add to cart</div>
        </button>
      `);
    });
  ...
  btnHoldSale.addEventListener('click', async ()=>{
    ...
    const payload = {
      cart: Array.from(cart.values()).map(it=>({ id: it.id, qty: it.qty })),
      form: {
        customer_id: customer.value || null,
        status: status.value,
        coupon_code: coupon.value || null,
    ...
    btn.addEventListener('click', async ()=>{
      ...
      for(const it of (p.cart || [])){
        ...
        if(hasProductOneEndpoint && productOneUrlTpl){
          ...
          const r = await fetch(oneUrl, { headers:{'Accept':'application/json'}});
