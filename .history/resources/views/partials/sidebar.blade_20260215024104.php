...
<div class="space-y-1">
  ...
  <div class="pl-3 pr-1 space-y-1 {{ $openProducts ? '' : 'hidden' }} overflow-hidden transition-[max-height] duration-300 ease-in-out"
       data-collapse-panel="products">
    <a href="{{ route('products.index') }}" class="{{ $baseChild }} {{ $is('products.index') ? $activeChild : $idleChild }}"> <svg width="100%" height="100%" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
 <path d="M21 12L14 5V9H3.8C3.51997 9 3.37996 9 3.273 9.0545C3.17892 9.10243 3.10243 9.17892 3.0545 9.273C3 9.37996 3 9.51997 3 9.8V14.2C3 14.48 3 14.62 3.0545 14.727C3.10243 14.8211 3.17892 14.8976 3.273 14.9455C3.37996 15 3.51997 15 3.8 15H14V19L21 12Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    <a href="{{ route('products.create') }}" class="{{ $baseChild }} {{ $is('products.create') ? $activeChild : $idleChild }}">Add Product</a>
    <a href="{{ route('products.categories') }}" class="{{ $baseChild }} {{ $is('products.categories') ? $activeChild : $idleChild }}">Category</a>
    <a href="{{ route('products.brands') }}" class="{{ $baseChild }} {{ $is('products.brands') ? $activeChild : $idleChild }}">Brands</a>
    <a href="{{ route('products.attributes') }}" class="{{ $baseChild }} {{ $is('products.attributes') ? $activeChild : $idleChild }}">Attributes</a>
    <a href="{{ route('products.reviews') }}" class="{{ $baseChild }} {{ $is('products.reviews') ? $activeChild : $idleChild }}">Reviews</a>
  </div>
  ...
  <div class="pl-3 pr-1 space-y-1 {{ $openCrm ? '' : 'hidden' }}"
       data-collapse-panel="crm">
    <a href="{{ route('crm.orders') }}" class="{{ $baseChild }} {{ $is('crm.orders') ? $activeChild : $idleChild }}">Orders</a>
    <a href="{{ route('crm.customers') }}" class="{{ $baseChild }} {{ $is('crm.customers') ? $activeChild : $idleChild }}">Customers</a>
    <a href="{{ route('crm.coupons') }}" class="{{ $baseChild }} {{ $is('crm.coupons') ? $activeChild : $idleChild }}">Coupons</a>
    <a href="{{ route('crm.reports') }}" class="{{ $baseChild }} {{ $is('crm.reports') ? $activeChild : $idleChild }}">Reports</a>
    <a href="{{ route('crm.payments') }}" class="{{ $baseChild }} {{ $is('crm.payments') ? $activeChild : $idleChild }}">Payments</a>
    <a href="{{ route('crm.taxes') }}" class="{{ $baseChild }} {{ $is('crm.taxes') ? $activeChild : $idleChild }}">Taxes</a>
    <a href="{{ route('crm.stock') }}" class="{{ $baseChild }} {{ $is('crm.stock') ? $activeChild : $idleChild }}">Stock</a>
    <a href="{{ route('crm.settings') }}" class="{{ $baseChild }} {{ $is('crm.settings') ? $activeChild : $idleChild }}">Settings</a>
  </div>
  ...
  <a href="{{ route('pos') }}"
    ...
    <span class="grid h-9 w-9 place-items-center rounded-2xl bg-amber-50 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300">
      <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <path d="M6 2h12v4H6z"/><path d="M6 10h12v12H6z"/><path d="M9 14h6"/>
      </svg>
    </span>
    <div class="text-left">
      <div>POS</div>
  ...
  <div class="pl-3 pr-1 space-y-1 {{ $openSettings ? '' : 'hidden' }}"
       data-collapse-panel="settings">
    <a href="{{ route('settings.general') }}" class="{{ $baseChild }} {{ $is('settings.general') ? $activeChild : $idleChild }}">General Setting</a>
    <a href="{{ route('settings.users') }}" class="{{ $baseChild }} {{ $is('settings.users') ? $activeChild : $idleChild }}">User Management</a>
    <a href="{{ route('settings.contact') }}" class="{{ $baseChild }} {{ $is('settings.contact') ? $activeChild : $idleChild }}">Contact Info</a>
    <a href="{{ route('settings.colors') }}" class="{{ $baseChild }} {{ $is('settings.colors') ? $activeChild : $idleChild }}">Color Combination</a>
    <a href="{{ route('settings.fonts') }}" class="{{ $baseChild }} {{ $is('settings.fonts') ? $activeChild : $idleChild }}">Font Family</a>
  </div>
