<script>
    ...
    const inner = panel.querySelector('[data-collapse-inner]') || panel.firstElementChild;
  ...
  function initCollapse(root) {
    root.querySelectorAll('[data-collapse-btn]').forEach(btn => {
      btn.addEventListener('click', () => {
        const key = btn.getAttribute('data-collapse-btn');
        const panel = root.querySelector(`[data-collapse-panel="${key}"]`);
        const chev  = btn.querySelector(`[data-collapse-chevron="${key}"]`);
        if (!panel) return;
  ...
  window.addEventListener('DOMContentLoaded', () => {
    // init on desktop sidebar + mobile drawer sidebar
    document.querySelectorAll('nav, #drawerPanel').forEach(root => initCollapse(root));
    ...
    document.querySelectorAll('[data-collapse-panel]').forEach(panel => {
      ...
      // if server default open exists (from blade), keep it
      const serverOpen = panel.getAttribute('data-collapse-default') === 'open';
      const shouldOpen = serverOpen || saved === 'open';

      setPanelOpen(panel, shouldOpen);

      document.querySelectorAll(`[data-collapse-chevron="${key}"]`).forEach(ch =>
        ch.classList.toggle('rotate-180', shouldOpen)
      );
      document.querySelectorAll(`[data-collapse-btn="${key}"]`).forEach(b =>
