...
<div class="space-y-1">
    ...
    <a href="{{ route('pos.barcode.labels') }}"
        class="{{ $baseLink }} {{ $is('pos.pos.barcode.*') ? $activeParent : $idleParent }}">
        <span class="grid h-9 w-9 place-items-center rounded-2xl bg-amber-50 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300">
            <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path d="M6 2h12v4H6z" />
                <path d="M6 10h12v12H6z" />
        ...
        <div class="text-left">
            <div>Barcode</div>
            <div class="text-xs opacity-70">Barcode Labels</div>
        </div>
    </a>
    ...
    <div
        ...
        <div data-collapse-inner class="pt-1 space-y-1">
            ...
            @if(auth()->user()?->canPerm('settings.users'))
            <a href="{{ route('settings.roles') }}" class="{{ $baseChild }} {{ request()->routeIs('settings.roles') ? $activeChild : $idleChild }}">
                <div class="flex items-center gap-2">
                    <span class="material-symbols-outlined text-[18px] leading-none opacity-70">remove</span>
